...
void demo_handler(int sig, siginfo_t *info, struct ucontext *ctx)
{
    ...
    fprintf(stderr, "GOT IT (signal %d, si_code %d)\n", sig, info->si_code);
    fprintf(stderr, "[INFO] Breakpoint hit at %p\n", info->si_addr);
...
#   define PRINT_REG32(x) fprintf(stderr, "[INFO]  "#x" = 0x%08lx  (s: %ld\tu: %lu)\n", CTXR(x), CTXR(x), CTXR(x))
#   define PRINT_FLAG(x) if (CTXR(eflags) & X86_EFLAGS_##x) fprintf(stderr, __STRING(x) " ")
    ...
    fprintf(stderr, "[INFO] "); PRINT_REG16(ss); PRINT_REG16(cs); fprintf(stderr, "\n");
    fprintf(stderr, "[INFO] "); PRINT_REG16(ds); PRINT_REG16(fs); fprintf(stderr, "\n");
    fprintf(stderr, "[INFO] "); PRINT_REG16(es); PRINT_REG16(gs); fprintf(stderr, "\n");
# else /* INDPENDENT */
// Instead, glibc defines indexes on an array
#   define CTXR(x) (ctx->uc_mcontext.gregs[REG_ ## x])
#   define PRINT_REG16(x) fprintf(stderr, " "#x" = 0x%02hx = %hu", CTXR(x), CTXR(x))
#   define PRINT_REG32(x) fprintf(stderr, "[INFO]  "#x" = 0x%08x  (s: %d\tu: %u)\n", CTXR(x), CTXR(x), CTXR(x))
...
# endif /* INDPENDENT */
    fprintf(stderr, "[INFO]  [ "); PRINT_FLAG(OF); PRINT_FLAG(DF); PRINT_FLAG(SF);
...
void before_entry()
{
    /* Program symbols are exposed as original_xxx */
    extern unsigned long original_prova();
    extern unsigned long original_prova_postinst();
    extern unsigned long original_provashort();
    extern unsigned long original_provashort_postinst();
    install_signal_handler();
    set_breakpoint((uintptr_t) original_prova, (uintptr_t) original_prova_postinst, demo_handler);
    set_breakpoint((uintptr_t) original_provashort, (uintptr_t) original_provashort_postinst, demo_handler);
